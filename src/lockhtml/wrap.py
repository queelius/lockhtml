"""Universal encrypted payload generation for lockhtml.
...
from .config import LockhtmlConfig
from .crypto import LockhtmlError, content_hash, encrypt
...
def wrap_file(
    ...
    Raises:
        LockhtmlError: If file cannot be read or encryption fails.
    """
    file_path = Path(file_path)
    if not file_path.is_file():
        raise LockhtmlError(f"File not found: {file_path}")

    # Read file bytes
    try:
        file_bytes = file_path.read_bytes()
    except OSError as e:
        raise LockhtmlError(f"Cannot read file {file_path}: {e}") from e
    ...
    try:
        ...
        raise LockhtmlError(f"Cannot write output {output_path}: {e}") from e
...
def wrap_site(
    ...
    Raises:
        LockhtmlError: If directory cannot be read or encryption fails.
    """
    dir_path = Path(dir_path)
    if not dir_path.is_dir():
        raise LockhtmlError(f"Directory not found: {dir_path}")
    ...
    if not file_list:
        raise LockhtmlError(f"Directory is empty: {dir_path}")
...
def _generate_wrap_html(
    ...
    site_js = ""
    if include_jszip:
        jszip_block = f"\n<script data-lockhtml-runtime>{_get_jszip_shim()}</script>"
        site_js = _get_site_renderer_js()

    marked_block = ""
    if include_marked:
        marked_block = f"\n<script data-lockhtml-runtime>{_get_marked_js()}</script>"
...
<head>
  ...
  <style data-lockhtml-runtime>{css}</style>
</head>
<body>
  <lockhtml-encrypt
    {attrs_str}>
  </lockhtml-encrypt>{jszip_block}{marked_block}
  <script data-lockhtml-runtime>
{crypto_js}
...
lockhtml-encrypt[data-decrypted] {{
  display: block;
  min-height: auto;
}}
...
.lockhtml-icon {{ font-size: 3rem; margin-bottom: 1rem; }}
.lockhtml-title {{ font-size: 1.25rem; font-weight: 600; margin-bottom: 0.5rem; color: #333; }}
.lockhtml-hint {{ color: #666; font-size: 0.9rem; margin-bottom: 1rem; }}
.lockhtml-filename {{ color: #999; font-size: 0.8rem; margin-bottom: 1rem; font-family: monospace; }}
...
.lockhtml-input:focus {{ border-color: {template.color_primary}; }}
...
.lockhtml-button:hover {{ opacity: 0.9; }}
.lockhtml-button:disabled {{ opacity: 0.5; cursor: not-allowed; }}
.lockhtml-error {{ color: #dc3545; font-size: 0.9rem; margin-top: 0.5rem; }}
...
/* Viewer base */
.lockhtml-viewer {{ width: 100%; }}
.lockhtml-viewer iframe {{ width: 100%; height: calc(100vh - 40px); border: none; }}
.lockhtml-viewer pre {{
  margin: 0;
...
.lockhtml-image-viewer img.zoomed {{
  max-width: none;
  max-height: none;
  cursor: zoom-out;
}}
...
.lockhtml-text-viewer {{
  display: flex;
  min-height: calc(100vh - 40px);
  background: #f5f5f5;
}}
.lockhtml-text-viewer .line-numbers {{
...
.lockhtml-download a:hover {{ opacity: 0.9; }}
.lockhtml-download .file-info {{ color: #666; font-size: 0.9rem; }}

/* Site viewer */
.lockhtml-site-frame {{ width: 100%; height: 100vh; border: none; }}
"""
...
(function() {
  ...
  window.__lockhtml = { decryptPayload, computeHash };
})();"""
...
def _get_renderer_js() -> str:
    ...
    return """
// lockhtml wrap renderer
(function() {
  'use strict';

  const el = document.querySelector('lockhtml-encrypt[data-encrypted]');
  if (!el) return;
  ...
  el.innerHTML = `
    <div class="lockhtml-container">
      ...
      <div class="lockhtml-title">Protected Content</div>
      <div class="lockhtml-filename">${filename}</div>
      <form class="lockhtml-form">
        ${isUserMode ? '<input type="text" class="lockhtml-input" placeholder="Username" autocomplete="username">' : ''}
        <input type="password" class="lockhtml-input lockhtml-password" placeholder="Password" autocomplete="current-password">
        <button type="submit" class="lockhtml-button">Decrypt</button>
  ...
  const pwdInput = el.querySelector('.lockhtml-password');
  const userInput = el.querySelector('input[placeholder="Username"]');
  const errorDiv = el.querySelector('.lockhtml-error');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const result = await window.__lockhtml.decryptPayload(encrypted, password, username);
    if (!result) {
    ...
    // Verify hash
    if (expectedHash) {
      const actualHash = await window.__lockhtml.computeHash(result.content);
    ...
    if (wrapType === 'site' && window.__lockhtml_renderSite) {
      window.__lockhtml_renderSite(el, bytes, meta);
    } else {
  ...
  function createToolbar(fname, size, downloadUrl) {
    ...
    toolbar.className = 'lockhtml-toolbar';
    toolbar.innerHTML =
  ...
  function renderImageViewer(viewer, url, fname) {
    viewer.className = 'lockhtml-viewer lockhtml-image-viewer';
  ...
  function renderTextViewer(viewer, text) {
    viewer.className = 'lockhtml-viewer lockhtml-text-viewer';
  ...
  function renderFile(container, bytes, meta) {
    ...
    viewer.className = 'lockhtml-viewer';
...
(function() {
  ...
  window.__lockhtml_ZipReader = ZipReader;
})();"""
...
(function() {
  ...
  window.__lockhtml_renderSite = async function(container, zipBytes, meta) {
