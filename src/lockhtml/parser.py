"""HTML parsing and transformation for lockhtml.

Handles finding <lockhtml-encrypt> elements, extracting content,
...
from .crypto import LockhtmlError, content_hash, decrypt, encrypt, rewrap_keys
...
def mark_elements(
    ...
    for selector in selectors:
        for element in soup.select(selector):
            # Note: We intentionally allow wrapping lockhtml-encrypt elements.
            ...
            # Skip if already wrapped in a lockhtml-encrypt element
            # (prevents double-wrapping the same content in one pass)
            if element.parent and element.parent.name == "lockhtml-encrypt":
                continue

            # Create wrapper element
            wrapper = soup.new_tag("lockhtml-encrypt")
            if hint:
                wrapper["hint"] = hint
            if title:
                wrapper["title"] = title
...
def find_lockhtml_elements(soup: BeautifulSoup) -> list[Tag]:
    """Find all <lockhtml-encrypt> elements in the document.

    Args:
        soup: Parsed HTML document.

    Returns:
        List of lockhtml-encrypt Tag elements.
    """
    return soup.find_all("lockhtml-encrypt")


def has_lockhtml_elements(html: str) -> bool:
    """Quick check if HTML contains lockhtml-encrypt elements.

    Args:
        html: HTML string to check.

    Returns:
        True if lockhtml-encrypt elements are present.
    """
    # Quick regex check before full parse
    return bool(re.search(r"<lockhtml-encrypt[\s>]", html, re.IGNORECASE))
...
def is_already_encrypted(element: Tag) -> bool:
    ...
    Args:
        element: lockhtml-encrypt Tag element.
...
def lock_html(
    ...
    Raises:
        LockhtmlError: If encryption fails or HTML is invalid.
    """
    if not has_lockhtml_elements(html):
    ...
    if encrypted_any:
        # Inject the lockhtml runtime into <head>
...
def unlock_html(
    ...
    Raises:
        LockhtmlError: If decryption fails or HTML is invalid.
    """
    if not has_lockhtml_elements(html):
        return html
...
def _inject_runtime(
    ...
    """Inject lockhtml JavaScript and CSS into the document head.
    ...
    existing = head.find("script", {"data-lockhtml-runtime": True})
    if existing:
        return
    ...
    style_tag["data-lockhtml-runtime"] = "true"
    style_tag.string = css_content
    head.append(style_tag)

    # Create script tag
    script_tag = soup.new_tag("script")
    script_tag["data-lockhtml-runtime"] = "true"
    script_tag.string = _get_javascript(template, defaults)
    head.append(script_tag)


def _remove_runtime(soup: BeautifulSoup) -> None:
    """Remove injected lockhtml runtime from the document.

    Args:
        soup: Parsed HTML document.
    """
    for tag in soup.find_all(attrs={"data-lockhtml-runtime": True}):
        tag.decompose()
...
lockhtml-encrypt[data-encrypted] {{
  min-height: 200px;
  display: flex;
  align-items: center;
...
.lockhtml-button:hover {{
  opacity: 0.9;
}}

.lockhtml-button:disabled {{
  opacity: 0.5;
  cursor: not-allowed;
}}
